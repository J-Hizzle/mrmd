...
void runCancelledLennardJones_idealGas_localCap(Config& config)
{
    // initialize
    ...
    action::CancelledLennardJones lennardJonesInner(
    ...
    for (auto step = 0; step < config.nsteps; ++step)
    {
        ...
        maxAtomDisplacement += langevinIntegrator.preForceIntegrate_apply_if_block_if(
            atoms, config.dt, isInThermostatRegion, isInInnerIntRegion);
...
int main(int argc, char* argv[])  // NOLINT
{
    ...
    runCancelledLennardJones_idealGas_localCap(config);
