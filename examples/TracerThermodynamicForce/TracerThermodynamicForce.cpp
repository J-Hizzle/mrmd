...
struct Config
{
    ...
    idx_t densityStart = 0;
    idx_t densityDeadTime = 0;
    idx_t densitySamplingInterval = 10;
    idx_t densityUpdateInterval = 2000;
    idx_t densityNumBins = 10;
...
void LJ(Config& config)
{
    ...
    // print x-grid for density
    for (auto i = 0; i < config.densityNumBins; ++i)
    {
        std::string append = (i < config.densityNumBins - 1) ? " " : "";
        auto posBin = subdomain.minCorner[0] +
                      (i + 0.5_r) * (subdomain.maxCorner[0] - subdomain.minCorner[0]) /
                          config.densityNumBins;  // construct grid along x-direction and shifted to
                                                  // the center of the density bins
        fDensityProfile << posBin << append;
    }
    ...
    for (auto step = 0; step < config.nsteps; ++step)
    {
        ...
        if (config.bOutput && (step % config.outputInterval == 0))
        {
            ...
            {
                std::string append = (i < config.densityNumBins - 1) ? " " : "";
                fDensityProfile << h_densityProfile(i, 0) << append;
            }
            fDensityProfile << std::endl;
