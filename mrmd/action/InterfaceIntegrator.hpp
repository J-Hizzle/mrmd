...
public:
    ...
    InterfaceIntegrator(const real_t& zeta, const real_t& temperature)
    {
    ...
    template <OnePositionPredicate Pred, OnePositionPredicate BlockPred>
    real_t preForceIntegrate_apply_if_block_if(data::Atoms& atoms,
                                                                        const real_t dt,
                                                                        const Pred& applyPred,
                                                                        const BlockPred& blockPred);
};

template <std::predicate<const real_t, const real_t, const real_t> UnaryPred>
real_t InterfaceIntegrator::preForceIntegrate_apply_if(data::Atoms& atoms,
                                                                    const real_t dt,
                                                                    const UnaryPred& pred)
{
    ...
    Kokkos::parallel_reduce("InterfaceIntegrator::preForceIntegrate",
                            policy,
                            kernel,
...
template <OnePositionPredicate Pred, OnePositionPredicate BlockPred>
real_t InterfaceIntegrator::preForceIntegrate_apply_if_block_if(data::Atoms& atoms,
                                                                    const real_t dt,
                                                                    const Pred& applyPred,
                                                                    const BlockPred& blockPred)
{
    ...
    auto kernel = KOKKOS_LAMBDA(const idx_t& idx, real_t& maxDistSqr)
    {
        ...
        if (overlap(idx))
        {
            if (!blockPred(dx[0], dx[1], dx[2]))
            {
                if (blockPred(pos(idx, 0), pos(idx, 1), pos(idx, 2)))
                {
                    action::updateDrift(
                        pos(idx, 0), pos(idx, 1), pos(idx, 2), -vel(idx, 0), -vel(idx, 1), -vel(idx, 2), dtHalf);
                }
        ...
        if (applyPred(pos(idx, 0), pos(idx, 1), pos(idx, 2)))
        {
            // Get a random number state from the pool for the active thread
            auto randGen = RNG.get_state();
